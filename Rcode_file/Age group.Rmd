...
```{r}
data<-Midus1_Squad_Data_USJAPANwellbeing%>%
  mutate(group = case_when(
    A1PRAGE_2019 >= 20 & A1PRAGE_2019 <= 36 ~ "YOUNGEST",
    A1PRAGE_2019 >= 37 & A1PRAGE_2019 <= 45 ~ "MIDDLE",
    A1PRAGE_2019 >= 46 ~ "OLDEST",
    TRUE ~ NA_character_
  ))
data_young<-data%>%
...
data_young <- left_join(data_young, Midus1_Squad_Data_USJAPANwellbeing, by = "M2ID")
data_middle <- left_join(data_middle, Midus1_Squad_Data_USJAPANwellbeing, by = "M2ID")
data_old <- left_join(data_old, Midus1_Squad_Data_USJAPANwellbeing, by = "M2ID")

```
YOUNG GROUP
```{r}
data_elbow<-data_old%>%
      dplyr::select("Zcommunion.x","ZMyMidus1Autonomy.x")%>%
      na.omit()
...
ggplot(data_young, aes(x = Zcommunion.x, y = ZMyMidus1Autonomy.x, color = Kmeans)) +
  geom_point(size = 1) +
...
ggplot(data_middle, aes(x = Zcommunion.x, y = ZMyMidus1Autonomy.x, color = Kmeans)) +
  geom_point(size = 1) +
  geom_jitter(width = 0.1, height = 0.1) +  
...
ggplot(data_old, aes(x = Zcommunion.x, y = ZMyMidus1Autonomy.x, color = Kmeans)) +
  geom_point(size = 1) +
  geom_jitter(width = 0.1, height = 0.1) +  
...
young
```{r}
means <- aggregate(A1SPWBS ~ Kmeans, data = data_young, FUN=mean)
ggplot(means, aes(x=Kmeans, y=A1SPWBS, fill=Kmeans)) +
  geom_bar(stat="identity", position=position_dodge()) +
  labs(title="Mean A1SPWBS by Kmeans Group", x="Kmeans Group", y="Mean A1SPWBS") +
  theme_minimal()
data_young$Kmeans <- as.factor(data_young$Kmeans)
anova_result <- aov(A1SPWBS ~ Kmeans, data = data_young)
...
middle
```{r}
means <- aggregate(A1SSATIS~ Kmeans, data = data_middle, FUN=mean)
ggplot(means, aes(x=Kmeans, y=A1SSATIS, fill=Kmeans)) +
  geom_bar(stat="identity", position=position_dodge()) +
  labs(title="Mean A1SSATIS by Kmeans Group", x="Kmeans Group", y="Mean A1SSATIS") +
  theme_minimal()
data_middle$Kmeans <- as.factor(data_middle$Kmeans)
anova_result <- aov(A1SSATIS ~ Kmeans, data = data_middle)
summary(anova_result)
tukey_test <- TukeyHSD(anova_result)
print(tukey_test)
plot(tukey_test, las=2)
...
old
```{r}
means <- aggregate(A1SPWBS ~ Kmeans, data = data_old, FUN=mean)
ggplot(means, aes(x=Kmeans, y=A1SPWBS, fill=Kmeans)) +
  geom_bar(stat="identity", position=position_dodge()) +
  labs(title="Mean A1SPWBS by Kmeans Group", x="Kmeans Group", y="Mean A1SPWBS") +
  theme_minimal()
data_old$Kmeans <- as.factor(data_old$Kmeans)
anova_result <- aov(A1SPWBS ~ Kmeans, data = data_old)
summary(anova_result)
tukey_test <- TukeyHSD(anova_result)
print(tukey_test)
plot(tukey_test, las=2)
